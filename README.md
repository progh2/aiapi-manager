...
- **요약 카드** — 기간 총 지출, 하루 평균, 최근 추세($/일), N일 뒤 누적 예측, 그룹 예산 소진 예상일
- **누적 지출과 예측** — 실선은 실제, 점선은 예측. 그룹 예산 합계가 지출과 비슷한 규모면 예산선도 함께 그린다
- **일별 지출** — 하루 단위 막대. 주말·과제 마감 같은 패턴이 드러난다
- **학생별 / 그룹별 지출** — 회색 트랙이 예산, 색 막대가 사용액. 예산을 넘긴 학생은 빨간 막대에 `초과` 표시
...
학기·캠프가 끝나면 키 목록에서 조/학급 또는 **만료됨 / 만료 임박(7일)** 필터로 모은 뒤
선택한 키를 한꺼번에 막는다.
